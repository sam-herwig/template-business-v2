...
const DEFAULT_SPLIT_OPTIONS: SplitTextAnimationOptions = {
  type: 'chars, words',
  stagger: 0.02,
  duration: 1,
  delay: 0,
 ...
 * 
 * NOTE: Options must be memoized by the caller (useMemo) or defined as a stable reference
 * to avoid re-running the animation on every render.
 */
export function useSplitTextAnimation(
  selector: string,
  options: SplitTextAnimationOptions = DEFAULT_SPLIT_OPTIONS
) {
  const splitRef = useRef<SplitText | null>(null)
  // Store options in a ref to avoid stale closure issues while still allowing updates
  ...
  }, [selector]) // Only re-run when selector changes, options are read from ref
